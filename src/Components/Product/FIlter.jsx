...
const {id,name}=useParams()
  ...
  const { isLoading, error, data } = useQuery({
    ...
    queryFn: async () => {
      try {
        const res = await axios.get("https://baseo.onrender.com/product-attributes/"+id);
        return res.data || [];
      } catch (error) {
  ...
  return (
    <section className="w-[100%] flex items-center justify-center overflow-hidden">
      <div className="w-[100%] flex gap-10 px-[6%] py-[3rem] flex-col">
        <p className="text-md text-gray-500"><a href="/" className="hover:text-green-500 duration-300"> Home </a>{'>'} {name} </p>
