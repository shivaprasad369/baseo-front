...
const {id,name,subname,subid}=useParams()
  // Fetch data using React Query
  const { isLoading, error, data } = useQuery({
    queryKey: ["product-sub"],
    queryFn: async () => {
      try {
        const res = await axios.get(`https://baseo.onrender.com/product-attributes/${id}/${subid}`);
        return res.data || [];
      } catch (error) {
  ...
  return (
    <section className="w-[100%] flex items-center justify-center overflow-hidden">
      <div className="w-[100%] flex gap-10 px-[6%] py-[3rem] flex-col">
        <p className="text-md text-gray-500"><a href="/" className="hover:text-green-500 duration-300"> Home </a>{'>'} {name} {'>'} {subname}</p>
          ...
          <div className="w-[70%] flex flex-col gap-5">
            ...
            >
              {filteredProducts.length > 0 ? (
                ...
                <div style={{ listStyle: "none", padding: 0 }} className="w-[100%] grid grid-cols-3 gap-6">
                  {filteredProducts.map((product, index) => (
                
                    <a href={`/products/details/${product.ProductID}/${encodeURIComponent(product.ProductName)}/${product.ProductAttributeID}`}
                      key={`${product.ProductAttributeID}-${index}`}
